...
export default function Signin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.data.message) {
      console.log(result.data.message.inboxes);
      ...
      sessionStorage.setItem(
        "userid",
        JSON.stringify(result.data.message.userid)
      );
      sessionStorage.setItem(
        "username",
        JSON.stringify(result.data.message.username)
      );
      dispatch({
        type: "SET_INBOXES",
        inboxes:
          result.data.message.inboxes == null
            ? []
            : result.data.message.inboxes,
      });
      navigate("/home");
    } else {
    ...
    <>
      <div className=" flex  min-h-screen items-center justify-center bg-gray-100">
        ...
        focus:ring-blue-500 focus:border-blue-500 focus:ring-1"
            ...
            <a className=' self-end   text-gray-600 dark:text-gray-400 decoration-2 hover:underline font-medium" href="#"'>
              forgot password?
            </a>
